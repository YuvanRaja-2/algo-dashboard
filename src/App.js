...
const css = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  :root {
    ...
    --border: rgba(255,255,255,0.06); --orange: #f97316; --orange2: #fb923c;
    --orange-glow: rgba(249,115,22,0.15); --green: #22c55e; --red: #ef4444;
    --text: #e2e8f0; --text2: #94a3b8; --text3: #475569; --blue: #3b82f6;
  }
  ...
  @keyframes glowPulse { 0%,100% { box-shadow: 0 0 0 0 rgba(249,115,22,0); } 50% { box-shadow: 0 0 20px 4px rgba(249,115,22,0.15); } }
  @keyframes dotPing { 0% { box-shadow: 0 0 0 0 rgba(34,197,94,0.4); } 70% { box-shadow: 0 0 0 6px rgba(34,197,94,0); } 100% { box-shadow: 0 0 0 0 rgba(34,197,94,0); } }
  @keyframes pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.4; } }
  @keyframes numberPop { 0% { opacity:0; transform: scale(0.8) translateY(8px); } 100% { opacity:1; transform: scale(1) translateY(0); } }
  @keyframes borderGlow { 0%,100% { border-color: rgba(255,255,255,0.06); } 50% { border-color: rgba(249,115,22,0.3); } }
  ...
  .anim-5 { animation: fadeUp 0.5s ease 0.5s both; }
  .anim-6 { animation: fadeUp 0.5s ease 0.6s both; }
  .slide-in { animation: slideIn 0.4s ease both; }
  .fade-in { animation: fadeIn 0.4s ease both; }

  /* SIDEBAR */
  .sidebar { width: 240px; min-width: 240px; background: var(--bg2); border-right: 1px solid var(--border); display: flex; flex-direction: column; padding: 24px 0; position: fixed; height: 100vh; overflow-y: auto; z-index: 100; animation: slideIn 0.4s ease both; }
  .logo { padding: 0 20px 24px; border-bottom: 1px solid var(--border); margin-bottom: 16px; }
  .logo-text { font-size: 20px; font-weight: 800; background: linear-gradient(135deg, #f97316, #fb923c); -webkit-background-clip: text; -webkit-text-fill-color: transparent; letter-spacing: -0.5px; }
  ...
  .nav-item { display: flex; align-items: center; gap: 10px; padding: 10px 12px; border-radius: 8px; cursor: pointer; transition: all 0.25s cubic-bezier(0.4,0,0.2,1); color: var(--text2); font-size: 13px; font-weight: 600; margin-bottom: 2px; }
  .nav-item:hover { background: var(--bg3); color: var(--text); transform: translateX(4px); }
  .nav-item.active { background: var(--orange-glow); color: var(--orange); border: 1px solid rgba(249,115,22,0.2); animation: glowPulse 3s infinite; }
  .nav-icon { font-size: 16px; width: 20px; text-align: center; }
  .sidebar-bottom { margin-top: auto; padding: 16px 12px 0; border-top: 1px solid var(--border); }
  .user-card { display: flex; align-items: center; gap: 10px; padding: 10px 12px; background: var(--bg3); border-radius: 10px; cursor: pointer; transition: all 0.2s; }
  .user-card:hover { background: var(--bg4); }
  .user-avatar { width: 32px; height: 32px; background: linear-gradient(135deg, #f97316, #fb923c); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: 800; color: white; }
  .user-name { font-size: 13px; font-weight: 700; }
  .user-role { font-size: 10px; color: var(--text3); font-family: 'DM Mono', monospace; }

  /* MAIN */
  .main { margin-left: 240px; flex: 1; display: flex; flex-direction: column; min-height: 100vh; }
  .topbar { background: var(--bg2); border-bottom: 1px solid var(--border); padding: 0 24px; height: 60px; display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; z-index: 50; animation: fadeIn 0.4s ease both; }
  .topbar-title { font-size: 16px; font-weight: 800; }
  .topbar-right { display: flex; align-items: center; gap: 12px; }
  .status-pill { display: flex; align-items: center; gap: 6px; padding: 6px 12px; border-radius: 999px; font-size: 11px; font-weight: 700; font-family: 'DM Mono', monospace; transition: all 0.2s; }
  .status-dot { width: 6px; height: 6px; border-radius: 50%; }
  .status-green { background: rgba(34,197,94,0.15); color: #22c55e; border: 1px solid rgba(34,197,94,0.2); }
  .status-green .status-dot { background: #22c55e; animation: pulse 2s infinite, dotPing 2s infinite; }
  ...
  .logout-btn { background: rgba(239,68,68,0.1); color: #ef4444; border: 1px solid rgba(239,68,68,0.2); padding: 6px 14px; border-radius: 8px; cursor: pointer; font-size: 12px; font-weight: 700; font-family: 'Syne', sans-serif; transition: all 0.2s; }
  .logout-btn:hover { background: rgba(239,68,68,0.2); transform: translateY(-1px); }

  /* CONTENT */
  .content { padding: 24px; flex: 1; }
  .stats-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 24px; }
  .stat-card { background: var(--bg2); border: 1px solid var(--border); border-radius: 14px; padding: 20px; position: relative; overflow: hidden; transition: all 0.25s cubic-bezier(0.4,0,0.2,1); cursor: default; }
  .stat-card:hover { border-color: rgba(249,115,22,0.3); transform: translateY(-4px); box-shadow: 0 12px 32px rgba(0,0,0,0.4), 0 0 0 1px rgba(249,115,22,0.1); }
  .stat-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, transparent, var(--orange), transparent); opacity: 0; transition: opacity 0.3s; }
  .stat-card:hover::before { opacity: 1; }
  .stat-label { font-size: 11px; color: var(--text3); font-family: 'DM Mono', monospace; margin-bottom: 8px; letter-spacing: 0.5px; }
  .stat-value { font-size: 24px; font-weight: 800; letter-spacing: -1px; animation: numberPop 0.5s ease both; }
  .stat-sub { font-size: 11px; color: var(--text3); margin-top: 4px; font-family: 'DM Mono', monospace; }
  .text-green { color: var(--green); } .text-red { color: var(--red); } .text-orange { color: var(--orange); } .text-blue { color: var(--blue); } .text-purple { color: #a855f7; }

  .grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px; }
  .grid-3 { display: grid; grid-template-columns: 2fr 1fr; gap: 16px; margin-bottom: 24px; }

  .card { background: var(--bg2); border: 1px solid var(--border); border-radius: 14px; overflow: hidden; transition: border-color 0.2s; }
  .card:hover { border-color: rgba(255,255,255,0.1); }
  .card-header { padding: 16px 20px; border-bottom: 1px solid var(--border); display: flex; align-items: center; justify-content: space-between; }
  .card-title { font-size: 13px; font-weight: 700; display: flex; align-items: center; gap: 8px; }
  .card-body { padding: 20px; }

  .signal-card { background: var(--bg3); border: 1px solid var(--border); border-radius: 12px; padding: 16px; margin-bottom: 12px; transition: all 0.25s cubic-bezier(0.4,0,0.2,1); }
  .signal-card:hover { border-color: rgba(249,115,22,0.4); transform: translateY(-2px); box-shadow: 0 4px 16px rgba(249,115,22,0.1); }
  ...
  .score-bar-bg { width: 100%; height: 4px; background: var(--bg4); border-radius: 999px; margin: 8px 0; overflow: hidden; }
  .score-bar { height: 4px; border-radius: 999px; transition: width 1s cubic-bezier(0.4,0,0.2,1); }
  ...
  .table-row { display: grid; grid-template-columns: 1fr 80px 80px 100px 100px 80px; gap: 8px; padding: 12px 16px; border-bottom: 1px solid var(--border); transition: all 0.2s; }
  .table-row:hover { background: var(--bg3); border-left: 2px solid var(--orange); padding-left: 14px; }
  ...
  .trade-item { display: flex; justify-content: space-between; align-items: center; padding: 10px 16px; border-bottom: 1px solid var(--border); transition: all 0.2s; }
  .trade-item:hover { background: var(--bg3); padding-left: 20px; }
  ...
  .session-card { background: var(--bg3); border: 1px solid var(--border); border-radius: 10px; padding: 14px 16px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center; transition: all 0.2s; }
  .session-card:hover { border-color: rgba(249,115,22,0.2); transform: translateX(4px); }
  ...
  .setting-row { display: flex; justify-content: space-between; align-items: center; padding: 12px 0; border-bottom: 1px solid var(--border); transition: background 0.2s; }
  .setting-row:hover { padding-left: 8px; }
  .setting-label { font-size: 13px; color: var(--text2); }
  .setting-input { background: var(--bg3); border: 1px solid var(--border); border-radius: 8px; padding: 6px 12px; color: var(--text); font-size: 13px; width: 120px; text-align: right; font-family: 'DM Mono', monospace; outline: none; transition: all 0.2s; }
  .setting-input:focus { border-color: var(--orange); box-shadow: 0 0 0 3px rgba(249,115,22,0.1); }
  ...
  .report-num { font-size: 40px; font-weight: 800; letter-spacing: -2px; animation: numberPop 0.6s ease both; }
  .report-label { font-size: 11px; color: var(--text3); font-family: 'DM Mono', monospace; margin-top: 4px; }
  .progress-bg { width: 100%; height: 6px; background: var(--bg4); border-radius: 999px; margin: 12px 0; overflow: hidden; }
  .progress-fill { height: 6px; border-radius: 999px; transition: width 1s cubic-bezier(0.4,0,0.2,1); }
  ...
  .login-glow { position: absolute; width: 500px; height: 500px; background: radial-gradient(circle, rgba(249,115,22,0.12), transparent 70%); border-radius: 50%; top: 50%; left: 50%; transform: translate(-50%, -50%); pointer-events: none; animation: glowPulse 4s infinite; }
  .login-card { background: var(--bg2); border: 1px solid var(--border); border-radius: 20px; padding: 40px; width: 400px; position: relative; z-index: 1; animation: fadeUp 0.6s ease both; }
  ...
  .login-input { width: 100%; background: var(--bg3); border: 1px solid var(--border); border-radius: 10px; padding: 12px 16px; color: var(--text); font-size: 14px; outline: none; transition: all 0.2s; box-sizing: border-box; font-family: 'Syne', sans-serif; }
  .login-input:focus { border-color: var(--orange); box-shadow: 0 0 0 3px rgba(249,115,22,0.1); }
  .login-btn { width: 100%; background: linear-gradient(135deg, #f97316, #fb923c); color: white; border: none; border-radius: 10px; padding: 14px; font-size: 15px; font-weight: 800; cursor: pointer; margin-top: 8px; font-family: 'Syne', sans-serif; transition: all 0.2s; letter-spacing: 0.5px; }
  .login-btn:hover { opacity: 0.9; transform: translateY(-2px); box-shadow: 0 8px 24px rgba(249,115,22,0.3); }
  ...
  .pair-row { display: flex; justify-content: space-between; align-items: center; padding: 12px 0; border-bottom: 1px solid var(--border); transition: all 0.2s; }
  .pair-row:hover { padding-left: 8px; }
  ...
  .recharts-area-area { filter: drop-shadow(0 4px 8px rgba(249,115,22,0.15)); }
    ...
    <div className="content">
      {account && (
        <div className="stats-grid">
          ...
          <div className="stat-card anim-4">
...
function ControlTab() {
  ...
  return (
    <div className="content">
      <div className="control-section anim-1">
        <div className="card">
          <div className="card-header">
              ...
              Object.entries(stats.by_symbol).map(([symbol, data]) => (
                <div key={symbol} style={{ marginBottom: 16 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 11, color: data.win_rate >= 50 ? 'var(--green)' : 'var(--red)' }}>{data.win_rate}%</span>
                  </div>
                  <div className="progress-bg">
